...
import usePayment from '../../hooks/usePayment'; // Ganti dengan usePayment


export default function MasterPayment() {
  const { data, loading, error, handlePostData, handleUpdateData } = usePayment(); // Gunakan fungsi akses API dari custom hook

  const newData = data?.map((menu) => ({
    id: menu.id,
    name: menu.nama_metode,
    image: menu.imageUrl,
  ...
  axios.patch(`${import.meta.env.VITE_API_URL}api/Payment/${id}/Activate`)
    .then((response) => {
