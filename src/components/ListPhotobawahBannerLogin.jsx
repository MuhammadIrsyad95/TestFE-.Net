...
//  
import useCourseLimitLogin from '../hooks/useCourseLimitLogin';

const ListPhotobawahBannerLogin = () => {
    const { data, loading, error } = useCourseLimitLogin();
                    ...
                    {data && data.map(product => (
                        <Grid item xs={6} lg={4} key={product.id}>
                            <Paper style={{ backgroundColor: '#fff', boxShadow: 'none', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', justifyContent: 'start', borderRadius: 0 }}>
                                <Link to ={`/detailclass/${product.idKategori}/${product.id}`} style={{ textDecoration: 'none' }}>
                                    <img src={`${import.meta.env.VITE_API_URL}product/${product.imageUrl}`} alt={product.deskripsi} style={{ width: '100%' }} />
                                </Link>
                                ...
                                <Typography variant="body1" component="p" color="textPrimary" style={{ fontWeight: 'bold', color: '#333', marginBottom: '20px' }}>
                                    {product.namaCourse}
                                </Typography>
...
ListPhotobawahBannerLogin.propTypes = {
...
export default ListPhotobawahBannerLogin;